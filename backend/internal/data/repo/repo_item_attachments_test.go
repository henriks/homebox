...
func TestAttachmentRepo_DeleteExternalLink_DoesNotRequireBlobStorage(t *testing.T) {
	...
	repos := New(tClient, tbus, config.Storage{PrefixPath: "/", ConnString: "mem://"}, "mem://{{ .Topic }}", config.Thumbnail{Enabled: false}, config.ImageScaling{Width: 1920, Height: 1920, Quality: 80})
	entity := useEntities(t, 1)[0]
