...
type Group struct {
	...
	// ScaleImages holds the value of the "scale_images" field.
	ScaleImages bool `json:"scale_images,omitempty"`
	...
	for i := range columns {
		...
		case group.FieldScaleImages:
			if value, ok := values[i].(*sql.NullBool); !ok {
				return fmt.Errorf("unexpected type %T for field scale_images", values[i])
			} else if value.Valid {
				_m.ScaleImages = value.Bool
			}
		default:
...
func (_m *Group) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("scale_images=")
	builder.WriteString(fmt.Sprintf("%v", _m.ScaleImages))
	builder.WriteByte(')')
